...
$pageTitle = 'Transport';
$pageSubTitle = 'Manage transport routes and vehicles';
...
<title>ATI Tangalle · Transport</title>
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-bus" style="color:var(--primary-light);margin-right:8px;"></i>Transport Management</h3>
                        <div class="panel-sub">Manage transport routes and vehicles</div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                        <div class="table-search">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="text" id="searchInput" placeholder="Search route, vehicle, driver...">
                        </div>
                        <button class="btn btn-primary" onclick="openAddModal()"><i class="fa-solid fa-plus"></i> Add Route</button>
                    </div>
                </div>
...
<div class="modal-overlay" id="transportModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Route</h3>
        ...
        <form id="transportForm">
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="route_name" name="route_name" class="form-control" placeholder="Route 3 - Airport Road" required>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label>Vehicle No.</label>
                        <input type="text" id="vehicle_no" name="vehicle_no" class="form-control" placeholder="WP-NA-1234">
                    </div>
                    <div class="form-group">
                        <label>Driver Name</label>
                        <input type="text" id="driver_name" name="driver_name" class="form-control" placeholder="Driver name">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="capacity" name="capacity" class="form-control" min="1" value="40">
                    </div>
                    <div class="form-group">
                        <label>Status</label>
                ...
                <div class="form-group">
                    <label>Stops</label>
                    <input type="text" id="stops" name="stops" class="form-control" placeholder="Main Gate, City Center, Bus Stand">
                </div>
            </div>
            <div class="modal-foot">
                <button type="button" class="btn btn-outline" onclick="closeModal()">Cancel</button>
                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save Route</button>
            </div>
        </form>
    </div>
</div>
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Route';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Route';
    document.getElementById('routeId').value = id;
    
    fetch(`api/transport_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const record = data.data;
                document.getElementById('route_name').value = record.route_name || '';
                document.getElementById('vehicle_no').value = record.vehicle_no || '';
                document.getElementById('driver_name').value = record.driver_name || '';
                document.getElementById('capacity').value = record.capacity || 40;
                document.getElementById('stops').value = record.stops || '';
...
function deleteTransport(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Error deleting route.', 'error');
        }
    })
    .catch(err => {
        showToast('Error deleting route. Please try again.', 'error');
        console.error('Error:', err);
    });
}
...
function loadTransport() {
    ...
    fetch(`api/transport_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--red);padding:40px;">
                    ...
                    ${data.message || 'Error loading routes'}
                </td></tr>`;
            ...
            if (records.length === 0) {
                tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:40px;">
                    <i class="fas fa-bus" style="font-size:32px;display:block;margin-bottom:10px;opacity:0.3;"></i>
            ...
            records.forEach(record => {
                const stops = record.stops ? record.stops.split(',').map(s => s.trim()).join(', ') : '—';
                
                html += `
                    <tr>
                        <td><strong>${record.route_name || '—'}</strong></td>
                        <td>${record.vehicle_no || '—'}</td>
                        <td>${record.driver_name || '—'}</td>
                        <td>${record.capacity || '—'}</td>
                        <td style="font-size:12px;color:var(--text-muted);">${stops}</td>
                        ...
                        <td>
                            <div class="actions" style="justify-content:center;">
                                ...
                                <button class="btn btn-danger btn-sm" onclick="deleteTransport(${record.id})">
        ...
        .catch(err => {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--red);padding:40px;">
                <i class="fas fa-exclamation-circle" style="font-size:24px;display:block;margin-bottom:10px;"></i>
                Error loading routes. Please refresh the page.
            </td></tr>`;
            console.error('Error:', err);
        });
}
