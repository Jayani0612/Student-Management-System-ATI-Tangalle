...
$pageTitle = 'Teachers';
$pageSubTitle = 'Manage teacher profiles, subjects and departments';
...
<title>ATI Tangalle · Teachers</title>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
<style>
/* ===== SAME CSS AS STUDENTS PAGE (copy from students.php) ===== */
:root {
    --primary: #2c6aa8;
...
.sidebar-brand .brand-text span { color: #6a9fd8; }
...
.sidebar-footer .logout-btn:hover i { color: #ef4444; }
...
.header-user:hover { background: var(--indigo-light); }
.header-user .avatar {
    width: 38px;
    height: 38px;
    border-radius: 50%;
...
@media (max-width: 768px) {
    ...
    .header-user { padding: 4px; background: transparent; }
}
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-chalkboard-teacher" style="color:var(--primary-light);margin-right:8px;"></i>Teacher Records</h3>
                        <div class="panel-sub">View and manage all teacher information</div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                        <div class="table-search">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="text" id="searchInput" placeholder="Search name, subject, email...">
                        </div>
                        <button class="btn btn-primary" onclick="openAddModal()"><i class="fa-solid fa-plus"></i> Add Teacher</button>
                    </div>
                </div>
                ...
                <div class="pagination">
                    <span id="resultCount">0 teachers</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal-overlay" id="teacherModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-user-plus" style="color:var(--primary-light);margin-right:8px;"></i>Add Teacher</h3>
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-user-plus" style="color:var(--primary-light);margin-right:8px;"></i>Add Teacher';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-user-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Teacher';
    document.getElementById('teacherId').value = id;
    
    fetch(`api/teachers_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('joined_date').value = teacher.joined_date || '';
                document.getElementById('teacherModal').classList.add('open');
            } else {
...
document.getElementById('teacherForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            closeModal();
            loadTeachers();
            showToast(data.message || 'Teacher saved successfully!', 'success');
        } else {
...
function deleteTeacher(id) {
    ...
    .catch(err => {
        showToast('Error deleting teacher. Please try again.', 'error');
        console.error('Error:', err);
    });
}
...
function loadTeachers() {
    ...
    fetch(`api/teachers_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                ...
                document.getElementById('resultCount').textContent = '0 teachers';
                return;
            }
            ...
            if (teachers.length === 0) {
                tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:40px;">
                    <i class="fas fa-chalkboard-teacher" style="font-size:32px;display:block;margin-bottom:10px;opacity:0.3;"></i>
                    No teachers found
                </td></tr>`;
                document.getElementById('resultCount').textContent = '0 teachers';
                return;
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions" style="justify-content:center;">
                                <button class="btn btn-warning btn-sm" onclick="openEditModal(${teacher.id})">
            ...
            document.getElementById('resultCount').textContent = `${teachers.length} teachers`;
        })
        .catch(err => {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--red);padding:40px;">
                <i class="fas fa-exclamation-circle" style="font-size:24px;display:block;margin-bottom:10px;"></i>
