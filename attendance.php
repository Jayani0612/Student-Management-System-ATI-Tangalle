...
$pageSubTitle = 'Track daily attendance records for students';

// Get students for dropdown
$students = mysqli_query($conn, "SELECT id, full_name, student_no FROM students ORDER BY full_name");
...
<title>ATI Tangalle · Attendance</title>
...
.data-table .status-badge.Present {
    background: var(--green-light);
    color: var(--green);
}

.data-table .status-badge.Absent {
    background: var(--red-light);
    color: var(--red);
}

.data-table .status-badge.Late {
    background: var(--orange-light);
    color: var(--orange);
}

.data-table .status-badge.Leave {
    background: var(--indigo-light);
    color: var(--primary);
}
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-clipboard-check" style="color:var(--primary-light);margin-right:8px;"></i>Attendance Records</h3>
                        <div class="panel-sub">Track daily attendance records for students</div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                        <div class="table-search">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="text" id="searchInput" placeholder="Search student name or number...">
                        </div>
                        <button class="btn btn-primary" onclick="openAddModal()"><i class="fa-solid fa-plus"></i> Add Attendance</button>
                    </div>
                </div>
...
<div class="modal-overlay" id="attendanceModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-clipboard-check" style="color:var(--primary-light);margin-right:8px;"></i>Add Attendance</h3>
                    ...
                    <div class="form-group">
                        ...
                        <select id="student_id" name="student_id" class="form-control" required>
                            ...
                            <?php while ($s = mysqli_fetch_assoc($students)): ?>
                                <option value="<?= $s['id'] ?>"><?= htmlspecialchars($s['student_no']) ?> - <?= htmlspecialchars($s['full_name']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="form-group">
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-clipboard-check" style="color:var(--primary-light);margin-right:8px;"></i>Add Attendance';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Attendance';
    document.getElementById('attendanceId').value = id;
    
    fetch(`api/attendance_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
...
document.getElementById('attendanceForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showToast(data.message || 'Error saving record.', 'error');
        }
    })
    .catch(err => {
        submitBtn.innerHTML = originalText;
...
function loadAttendance() {
    ...
    fetch(`api/attendance_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
            ...
            if (records.length === 0) {
                tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:40px;">
                    <i class="fas fa-clipboard-check" style="font-size:32px;display:block;margin-bottom:10px;opacity:0.3;"></i>
            ...
            records.forEach(record => {
                ...
                html += `
                    <tr>
                        <td><strong>${record.student_no || '—'}</strong></td>
            ...
            document.getElementById('resultCount').textContent = `${records.length} records`;
        })
        .catch(err => {
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--red);padding:40px;">
                <i class="fas fa-exclamation-circle" style="font-size:24px;display:block;margin-bottom:10px;"></i>
