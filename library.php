...
$pageTitle = 'Library';
$pageSubTitle = 'Manage the book catalog and copy availability';
...
<title>ATI Tangalle · Library</title>
...
.data-table .copy-badge.Available {
    background: var(--green-light);
    color: var(--green);
}

.data-table .copy-badge.Limited {
    background: var(--orange-light);
    color: var(--orange);
}

.data-table .copy-badge.Unavailable {
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-book" style="color:var(--primary-light);margin-right:8px;"></i>Library Catalog</h3>
                        <div class="panel-sub">Manage the book catalog and copy availability</div>
                    </div>
...
<button class="btn btn-primary" onclick="openAddModal()">.                    </div>
                </div>
...
<div class="modal-overlay" id="libraryModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Book</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="total_copies" name="total_copies" class="form-control" min="1" value="1">
                    </div>
                </div>
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Book';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Book';
    document.getElementById('bookId').value = id;
    
    fetch(`api/library_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('total_copies').value = record.total_copies || 1;
                document.getElementById('available_copies').value = record.available_copies || 1;
                document.getElementById('libraryModal').classList.add('open');
            } else {
...
function loadLibrary() {
    ...
    fetch(`api/library_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--red);padding:40px;">
                    ...
                    ${data.message || 'Error loading books'}
                </td></tr>`;
            ...
            records.forEach(record => {
                const availability = record.total_copies > 0 
                    ? Math.round((record.available_copies / record.total_copies) * 100)
                    : 0;
                ...
                html += `
                    <tr>
                        ...
                        <td>${record.author || '—'}</td>
                        <td><span style="background:var(--bg);padding:2px 10px;border-radius:12px;font-size:12px;">${record.category || '—'}</span></td>
                        <td style="font-size:12px;color:var(--text-muted);">${record.isbn || '—'}</td>
                        <td>
                            <div style="display:flex;align-items:center;gap:8px;">
                                <span>${record.available_copies}/${record.total_copies}</span>
                                <span class="copy-badge ${badgeClass}">${badgeText}</span>
                            </div>
                        </td>
                        <td>
                            <div class="actions" style="justify-content:center;">
                                ...
                                <button class="btn btn-danger btn-sm" onclick="deleteLibrary(${record.id})">
        ...
        .catch(err => {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--red);padding:40px;">
                <i class="fas fa-exclamation-circle" style="font-size:24px;display:block;margin-bottom:10px;"></i>
                Error loading books. Please refresh the page.
            </td></tr>`;
            console.error('Error:', err);
        });
}
