...
$pageTitle = 'Finance';
$pageSubTitle = 'Manage fee invoices and payment status';

// Get students for dropdown
$students = mysqli_query($conn, "SELECT id, student_no, full_name FROM students ORDER BY full_name");
$studentRows = [];
...
<title>ATI Tangalle · Finance</title>
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-coins" style="color:var(--primary-light);margin-right:8px;"></i>Finance Management</h3>
                        <div class="panel-sub">Manage fee invoices and payment status</div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                        <div class="table-search">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="text" id="searchInput" placeholder="Search invoice or student...">
                        </div>
                        <button class="btn btn-primary" onclick="openAddModal()"><i class="fa-solid fa-plus"></i> Add Payment</button>
                    </div>
                </div>
...
<div class="modal-overlay" id="financeModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Payment</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="invoice_no" name="invoice_no" class="form-control" placeholder="INV-5001" required>
                    </div>
                    <div class="form-group">
                        <label>Amount ($) *</label>
                        <input type="number" step="0.01" id="amount" name="amount" class="form-control" placeholder="850.00" required>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="student_id" name="student_id" class="form-control" required>
                        ...
                        <?php foreach ($studentRows as $s): ?>
                            <option value="<?= $s['id'] ?>"><?= htmlspecialchars($s['student_no'] . ' - ' . $s['full_name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>Notes</label>
                    <input type="text" id="notes" name="notes" class="form-control" placeholder="e.g. Semester tuition fee">
                </div>
            </div>
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Payment';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Payment';
    document.getElementById('financeId').value = id;
    
    fetch(`api/finance_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const record = data.data;
                document.getElementById('invoice_no').value = record.invoice_no || '';
                document.getElementById('student_id').value = record.student_id || '';
                document.getElementById('amount').value = record.amount || '';
                document.getElementById('payment_date').value = record.payment_date || '';
...
document.getElementById('financeForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            closeModal();
            loadFinance();
            showToast(data.message || 'Payment saved successfully!', 'success');
        } else {
            showToast(data.message || 'Error saving payment.', 'error');
        }
    })
    .catch(err => {
        submitBtn.innerHTML = originalText;
...
function loadFinance() {
    ...
    fetch(`api/finance_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--red);padding:40px;">
                    ...
                    ${data.message || 'Error loading payments'}
                </td></tr>`;
            ...
            if (records.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:40px;">
                    <i class="fas fa-coins" style="font-size:32px;display:block;margin-bottom:10px;opacity:0.3;"></i>
            ...
            records.forEach(record => {
                const date = record.payment_date ? new Date(record.payment_date).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric'
                }) : '—';
                ...
                html += `
                    <tr>
                        <td><strong>${record.invoice_no || '—'}</strong></td>
                        <td>
                            <strong>${record.student_name || '—'}</strong>
                            <br><small style="color:var(--text-muted);font-size:11px;">${record.student_no || ''}</small>
                        </td>
                        <td><strong>$${parseFloat(record.amount || 0).toFixed(2)}</strong></td>
                        <td>${date}</td>
                        <td><span class="status-badge ${record.status || 'Pending'}">${record.status || 'Pending'}</span></td>
                        <td>
                            <div class="actions" style="justify-content:center;">
                                <button class="btn btn-warning btn-sm" onclick="openEditModal(${record.id})">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn btn-danger btn-sm" onclick="deleteFinance(${record.id})">
