...
$pageTitle = 'Timetable';
$pageSubTitle = 'Manage the weekly class schedule';
...
<title>ATI Tangalle · Timetable</title>
...
.data-table .day-badge.Monday { background: #e8f0fe; color: #4a90d9; }
.data-table .day-badge.Tuesday { background: #d1fae5; color: #10b981; }
.data-table .day-badge.Wednesday { background: #fef3c7; color: #f59e0b; }
.data-table .day-badge.Thursday { background: #ede9fe; color: #8b5cf6; }
.data-table .day-badge.Friday { background: #fce7f3; color: #ec4899; }
.data-table .day-badge.Saturday { background: #dbeafe; color: #3b82f6; }
.data-table .day-badge.Sunday { background: #fee2e2; color: #ef4444; }
        ...
        <div class="content">
            <div class="card panel">
                <div class="table-toolbar">
                    <div>
                        <h3 class="panel-title"><i class="fas fa-clock" style="color:var(--primary-light);margin-right:8px;"></i>Timetable Schedule</h3>
                        <div class="panel-sub">Manage the weekly class schedule</div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                        <div class="table-search">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="text" id="searchInput" placeholder="Search course or room...">
                        </div>
                        <button class="btn btn-primary" onclick="openAddModal()"><i class="fa-solid fa-plus"></i> Add Schedule</button>
                    </div>
                </div>
...
<div class="modal-overlay" id="timetableModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="modalTitle"><i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Schedule</h3>
            <button class="modal-close" onclick="closeModal()">&times;</button>
        </div>
        <form id="timetableForm">
            <div class="modal-body">
                <input type="hidden" id="ttId" name="id">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Room</label>
                        <input type="text" id="room" name="room" class="form-control" placeholder="Room 204">
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="end_time" name="end_time" class="form-control" value="09:00">
                    </div>
                </div>
            </div>
...
function openAddModal() {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle" style="color:var(--primary-light);margin-right:8px;"></i>Add Schedule';
...
function openEditModal(id) {
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit" style="color:var(--primary-light);margin-right:8px;"></i>Edit Schedule';
    document.getElementById('ttId').value = id;
    
    fetch(`api/timetable_api.php?action=get&id=${id}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const record = data.data;
                document.getElementById('course_id').value = record.course_id || '';
                document.getElementById('day_of_week').value = record.day_of_week || 'Monday';
                document.getElementById('start_time').value = record.start_time || '08:00';
                document.getElementById('end_time').value = record.end_time || '09:00';
...
function loadTimetable() {
    ...
    fetch(`api/timetable_api.php?action=list&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--red);padding:40px;">
                    ...
                    ${data.message || 'Error loading schedule'}
                </td></tr>`;
            ...
            if (records.length === 0) {
                tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:40px;">
                    <i class="fas fa-clock" style="font-size:32px;display:block;margin-bottom:10px;opacity:0.3;"></i>
            ...
            records.forEach(record => {
                ...
                html += `
                    <tr>
                        <td><span class="day-badge ${record.day_of_week}">${record.day_of_week}</span></td>
                        <td>${time}</td>
                        <td><strong>${record.course_code || '—'}</strong><br><small>${record.course_name || ''}</small></td>
                        <td>${record.room || '—'}</td>
                        <td>
                            <div class="actions" style="justify-content:center;">
                                ...
                                <button class="btn btn-danger btn-sm" onclick="deleteTimetable(${record.id})">
